...
Cookies = {"RBXEventTrackerV2": ""} # import RBXEventTrackerV2 cookie in the 2nd quotation mark set
...
def DecodeMetadata(MetadataB64: str) -> dict:
    return Base64.b64decode(MetadataB64).decode("utf-8")

def FetchToken(DataExchange: str):
    Data = {
        "bda": ROBLOX_BDA,
        "capi_version": "2.2.2",
        "capi_mode": "inline",
...
def FinishLogin(ChallengeId: str, MetadataB64: str, CSRF: str, Data: str):
    Headers = {
...
def StartLogin():
    ...
    print(f"[MAGNET] FunCaptcha challenge token: {Token}")

    print("[MAGNET] Continuing FunCaptcha challenge")
    if ContinueCaptcha(ChallengeId, Metadata=Metadict, Headers=Headers) == True:
        print("[MAGNET] FunCaptcha challenge continued successfully")
    else:
        print("[MAGNET] FunCaptcha challenge didn't continue")
    ...
    FinishLogin(ChallengeId, MetadataB64=MetadataB64, CSRF=CSRF, Data=Data)
